...
assert(
  base && /^https:\/\/[^/]+\.pages\.dev\/?$/.test(base),
  "Usage: node scripts/verify-deploy.cjs https://PROJECT.pages.dev",
);
...
(async () => {
  ...
  for (const file of [
    ...
    assert.equal(
      hash(remote),
      hash(fs.readFileSync("dist/" + file)),
      `Deployed ${file} differs from the verified release`,
    );
  }
  for (const file of [
    ".env.local",
    ...
    assert.equal(
      response.status,
      404,
      `Private/development path should return 404: ${file}`,
    );
  }
  const audio = await fetch(`${root}/assets/audio/learn-jp.mp3`, {
  ...
  assert(
    [200, 206].includes(audio.status),
    "Audio must return a full file or a valid partial response",
  );
  assert(
  ...
  if (audio.status === 206) {
    assert(range.startsWith("bytes 0-1023/"));
    assert.equal(audioBytes.length, 1024);
  } else {
    assert.equal(
      createHash("sha256").update(audioBytes).digest("hex"),
      createHash("sha256")
        .update(fs.readFileSync("dist/assets/audio/learn-jp.mp3"))
        .digest("hex"),
      "Complete audio content must match the release",
    );
  }
  const cache = audio.headers.get("cache-control") || "";
  assert.equal(
    (cache.match(/max-age=/g) || []).length,
    1,
    "Cache policy must not contain conflicting max-age directives",
  );
  const report = {
    url: root,
    version: build.version,
    publicFilesChecked: checked.length,
    entrypointHashesMatch: true,
    securityHeaders: true,
    privateFilesReturn404: true,
    audioDeliveryStatus: audio.status,
    audioContentVerified: true,
    audioRange: range,
    verifiedAt: new Date().toISOString(),
  };
  fs.writeFileSync(
    "evidence/deployed-assets.json",
