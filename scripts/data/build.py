...
source_meta={'label':'mledoze country metadata','url':f'https://github.com/mledoze/countries/blob/{COUNTRY_PIN}/countries.json','snapshot':f'pinned {COUNTRY_PIN[:12]}, retrieved {DATE}'}
...
for raw in base:
    ...
    catalog.append({
        ...
        'borderCodes':neighbors,'uncertainBorderCodes':[],'borderNote':curated['borderNotes'].get(code,'Land-border connections follow the cited country dataset; sea neighbors are different.'),
...
# French Guiana is an integral part of France. Confirm its overseas neighbors
# against the same pinned metadata rather than treating them as wrong answers.
for a,b in curated.get('extraBorders',[]):
    assert a=='fr' and by_code[b]['cca3'] in by_code['gf']['borders'], (a,b)
    for first,second in [(a,b),(b,a)]:
        if second not in by[first]['borderCodes']: by[first]['borderCodes'].append(second)
        if first!='fr': by[first]['borderNote']+=' The border with France is through French Guiana, an integral part of France in South America.'
for a,b in curated.get('uncertainBorders',[]):
    by[a]['uncertainBorderCodes'].append(b)
    by[b]['uncertainBorderCodes'].append(a)
for c in catalog: c['borderCodes'].sort()
(ROOT/'data/catalog.json').write_text(json.dumps(catalog,ensure_ascii=False,indent=2)+'\n')
(ROOT/'data/provenance.json').write_text(json.dumps({'retrievedAt':DATE,'factbookCommit':PIN,'countryCommit':COUNTRY_PIN,'timeZoneVersion':source_tz['snapshot'],'entries':len(catalog),'flagStories':len(catalog),'religionNotes':sum(c['beliefs'] is not None for c in catalog),'asymmetricBordersExcluded':asymmetric,'sources':[source_meta,source_tz,source_geo]},ensure_ascii=False,indent=2)+'\n')
...
world=json.loads((CACHE/'world.geojson').read_text())
world_paths=[{'id':f['properties']['ADM0_A3'],'continent':f['properties']['CONTINENT'],'d':svg_path(f['geometry'])} for f in world['features'] if f['properties']['ADM0_A3']!='ATA']
(ROOT/'data/world-map.json').write_text(json.dumps(world_paths,separators=(',',':'))+'\n')
...
for feature in geojson['features']:
    ...
    outlines[name]=svg_path(feature['geometry'])
