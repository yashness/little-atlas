...
export function regionLabel(
  country: Pick<Country, "regions" | "regionMode">,
): string {
  return country.regions.join(
    country.regionMode === "convention" ? " or " : " & ",
  );
}
export function regionSentence(country: Country): string {
  const label = regionLabel(country);
  if (country.regionMode === "spans")
    return `${country.name} spans ${label}. Both belong in its answer.`;
  if (country.regionMode === "convention")
    return `${country.name} is grouped with ${label}, depending on the convention. We accept both.`;
  return `${country.name} is in ${label}.`;
}
export function sharesRegion(a: Country, b: Region): boolean {
  return a.regions.includes(b);
}
...
export function utcOffset(zone: string, at: Date): number | null {
  if (cachedInstant !== at.getTime()) {
    zoneOffsets.clear();
    cachedInstant = at.getTime();
  }
  ...
  try {
    ...
    /* older device tzdb: never guess from longitude */
  }
...
export function offsets(country: Country, at: Date): number[] {
  return unique(
    country.timeZones
      .map((zone) => utcOffset(zone, at))
      .filter((n): n is number => n !== null),
  ).sort((a, b) => a - b);
}
export function clockDistance(a: Country, b: Country, at: Date): number | null {
  if (
    [...a.timeZones, ...b.timeZones].some(
      (zone) => utcOffset(zone, at) === null,
    )
  )
    return null;
  const first = offsets(a, at),
    second = offsets(b, at);
  return first.length && second.length
    ? Math.min(...first.flatMap((x) => second.map((y) => Math.abs(x - y))))
    : null;
}
...
export function localClock(zone: string, at: Date): string {
  try {
    ...
    return "Clock unavailable on this device";
  }
}
